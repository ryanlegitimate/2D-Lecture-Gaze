...
import numpy as np
from collections import deque  # reused queue
...
# Enable OpenGaze streams (NO BLINK STREAMS)
for cmd in [
    '<SET ID="ENABLE_SEND_TIME" STATE="1" />',
    '<SET ID="ENABLE_SEND_TIME_TICK" STATE="1" />',      # time tick
    ...
    while _run_gaze:
        try:
            ...
            while b"\r\n" in buf:
                ...
                # enqueue sample (NO blink fields)
                gaze_queue.append({
                    "ts": time.time(),
...
def draw_overlay(img):
    if gaze["BPOGV"] == 1:
        ...
        cv2.circle(img, (bx,by), 20, (255,0,0), 3)  # binocular only
...
# --- Biometrics HUD (top-right): HR, GSR, engagement wheel (NO blinks) ---
def draw_biometrics_hud(img):
    ...
    # Engagement wheel (uses DIAL/DIALV)
    dial_val = gaze["DIAL"]
...
wr.writerow([
    ...
    # Biometrics only (NO blink fields):
...
def draw_overlay(frame):
    if gaze["BPOGV"] == 1:
        bx, by = int(gaze["BPOGX"]*w), int(gaze["BPOGY"]*h)
        cv2.circle(frame, (bx,by), 20, (255,0,0), 3)  # binocular only
...
def draw_pupil_hud(frame):
    ...
    cv2.putText(frame, f"L {ltxt}   |   R {rtxt}", (28, 40),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,255), 2, cv2.LINE_AA)
    cv2.putText(frame, f"Valid L:{gaze['LPUPILV']} R:{gaze['RPUPILV']}", (28, 62),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200,200,200), 1, cv2.LINE_AA)

def draw_biometrics_hud_play(frame):
    draw_biometrics_hud(frame)  # top-right; HR/GSR + engagement only
...
while True:
    ...
    # drain all samples (≈150 Hz) to CSV
    while gaze_queue:
    ...
    if researcher_mode:
        ...
        draw_biometrics_hud_play(display_frame)  # HR/GSR + engagement (top-right)
