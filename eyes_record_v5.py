...
# Correct OpenGaze stream enables
for cmd in [
    ...
    '<SET ID="ENABLE_SEND_PUPIL_LEFT" STATE="1" />',   # LPD/RPD in pixels
    '<SET ID="ENABLE_SEND_PUPIL_RIGHT" STATE="1" />',
    '<SET ID="ENABLE_SEND_EYE_LEFT" STATE="1" />',     # LPUPILD (meters)
...
def draw_overlay(img):
    ...
    if gaze["LPOGV"] == 1:
        ...
        cv2.circle(img, (lx,ly), 10, (0,0,255), -1)  # red
    if gaze["RPOGV"] == 1:
        rx, ry = int(gaze["RPOGX"]*w), int(gaze["RPOGY"]*h)
        cv2.circle(img, (rx,ry), 10, (0,255,0), -1)  # green
...
def draw_splash(w,h, participant, researcher_mode):
    ...
    # --- LIVE GAZE ON SPLASH (only when Researcher Mode is ON) ---
    if researcher_mode:
        overlay = img.copy()
        ...
        # RM OFF: show clean splash (no dots, no HUD)
        return img
...
while True:
    ...
    # two buffers: one for recording (always with overlays), one for display (conditional)
    record_frame  = frame.copy()
    display_frame = frame.copy()

    # time since video start
    t_rel = time.time() - t0
    lp_mm = gaze["LPUPILD_m"]*1000 if gaze["LPUPILD_m"]>0 else ""
    rp_mm = gaze["RPUPILD_m"]*1000 if gaze["RPUPILD_m"]>0 else ""

    # log to CSV
    wr.writerow([
        participant, round(t_rel,3),
        gaze["LPOGX"], gaze["LPOGY"], gaze["LPOGV"],
        gaze["RPOGX"], gaze["RPOGY"], gaze["RPOGV"],
        gaze["BPOGX"], gaze["BPOGY"], gaze["BPOGV"],
        gaze["LPD_px"], gaze["RPD_px"],
    ...
    if researcher_mode:
        ...
        draw_pupil_hud(display_frame)     # HUD only in Researcher Mode
